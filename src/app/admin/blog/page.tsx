...
function PostForm({ p }: { p?: { id: string; slug: string; title: string; excerpt: string; body: string[]; coverImage: string | null; published: boolean; date: Date } }) {
  return (
    <form action={savePost} className="space-y-3 rounded-xl border border-ivory-200 bg-white p-5">
      ...
      <label className="block text-sm">Body <span className="text-xs text-foreground/45">(separate paragraphs with a blank line)</span><textarea name="body" rows={8} defaultValue={(p?.body ?? []).join("\n\n")} className={input} /></label>
  ...
  return (
    <div className="p-8">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div><h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-foreground/55">New post</h2><PostForm /></div>
        <div className="space-y-4">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/55">Existing ({posts.length})</h2>
          {posts.map((p) => <details key={p.id} className="rounded-xl border border-ivory-200 bg-white"><summary className="cursor-pointer px-5 py-3 font-medium text-brand-800">{p.title} {!p.published && <span className="text-xs text-amber-600">(draft)</span>}</summary><div className="p-4 pt-0"><PostForm p={p} /></div></details>)}
        </div>
      </div>
    </div>
  );
