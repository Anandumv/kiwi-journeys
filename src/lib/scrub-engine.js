...
function mountScrollWorld(container, config) {
  // Force reduce to false so the video always loads, even in Low Power Mode
  const reduce = false;
  ...
  function layout() {
    ...
    totalW = off;
    track.style.height = (totalW * vh) + 'px';
    read();
  }
  ...
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        // Safari sometimes fails to fire `seeked` for Blob videos at time 0.
        // Revealing on `loadeddata` ensures the video is always shown.
