...
- Railway schedules cron per service, so a single-service deployment has no cron. The jobs run from `src/instrumentation.ts`, which calls the app's own `/api/cron/*` routes over loopback with `CRON_SECRET`, gated on `ENABLE_INTERNAL_CRON=true`. Every job is now replica-safe: notification jobs claim rows and queue emails through `enqueueUniqueEmails` with ids such as `reminder-24h-<bookingId>`, `survey-<bookingId>`, `recovery-<reservationId>` and `loyalty-<customerId>`, so repeated or overlapping runs add nothing. Before this fix, hourly reminders over 4-hour windows sent each reminder about 4 times, and daily surveys over a 2-day window sent twice. API rate limits are still process-local.
...
- Waitlist notification availability subtracts active, unexpired reservation `seats` as well as confirmed bookings. An available entry is claimed (`updateMany where notified=false`) and its email queued via `enqueueEmails` in one transaction; deliver-emails sends it with retries and an idempotency key. `notified` therefore means "queued".
...
- Uploads persist through `UPLOAD_DIR` (Railway volume mounted at `/data`, `UPLOAD_DIR=/data/uploads`). `lib/upload.ts#uploadDir()` is the single source for write and serve paths. Files committed under `public/uploads` still serve statically. `BLOB_READ_WRITE_TOKEN` takes precedence when set.
- Production already uses `NEXT_PUBLIC_BASE_URL=https://kiwi-journeys-production.up.railway.app`. `kiwiglobetours.co.nz` is a separate live nginx site, so do not repoint DNS without the owner.
